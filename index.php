        ...
        <div class="content">
            <div class="newsfeed-container">
                <ul class="category-container">
                    <li class=<?= $selectedCat ? '' : 'cat-active' ?>><a href="http://localhost:8888/blogEval/">Tous les articles <span class="small">(<?= count($articles) ?>)</span></a></li>
                    <?php foreach ($categories as $catName => $catNum) : ?>
                        <li class=<?= $selectedCat ===  $catName ? 'cat-active' : '' ?>><a href="http://localhost:8888/blogEval/?cat=<?= $catName ?>"> <?= $catName ?><span class="small">(<?= $catNum ?>)</span> </a></li>
                    <?php endforeach; ?>
                </ul>
                        ...
                        <?php foreach ($categories as $cat => $num) : ?>
                            ...
                            <div class="articles-container">
                                <?php foreach ($articlePerCategories[$cat] as $a) : ?>
                                    <a href="http://localhost:8888/blogEval/show-article.php?id=<?= $a['id'] ?>" class="article block">
                                        <div class="overflow">
